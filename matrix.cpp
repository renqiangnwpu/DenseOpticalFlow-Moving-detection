...
static FLOAT_LIBVISO sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrarg*sqrarg)
static FLOAT_LIBVISO maxarg1,maxarg2;
...
Matrix::Matrix (const int32_t m_,const int32_t n_,const FLOAT_LIBVISO* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
Matrix::Matrix (const Matrix &M) {
  ...
  for (int32_t i=0; i<M.m; i++)
    memcpy(val[i],M.val[i],M.n*sizeof(FLOAT_LIBVISO));
}
...
void Matrix::getData(FLOAT_LIBVISO* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(FLOAT_LIBVISO s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(FLOAT_LIBVISO s,int32_t i1,int32_t i2) {
...
Matrix Matrix::rotMatX (const FLOAT_LIBVISO &angle) {
  FLOAT_LIBVISO s = sin(angle);
  FLOAT_LIBVISO c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
  R.val[1][1] = +c;
  R.val[1][2] = -s;
...
Matrix Matrix::rotMatY (const FLOAT_LIBVISO &angle) {
  FLOAT_LIBVISO s = sin(angle);
  FLOAT_LIBVISO c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const FLOAT_LIBVISO &angle) {
  FLOAT_LIBVISO s = sin(angle);
  FLOAT_LIBVISO c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::operator* (const FLOAT_LIBVISO &s) {
  Matrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
FLOAT_LIBVISO Matrix::l2norm () {
  FLOAT_LIBVISO norm = 0;
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
...
FLOAT_LIBVISO Matrix::mean () {
  FLOAT_LIBVISO mean = 0;
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      mean += val[i][j];
  return mean/(FLOAT_LIBVISO)(m*n);
}
...
bool Matrix::solve (const Matrix &M, FLOAT_LIBVISO eps) {
  ...
  FLOAT_LIBVISO big, dum, pivinv, temp;
...
bool Matrix::lu(int32_t *idx, FLOAT_LIBVISO &d, FLOAT_LIBVISO eps) {
  ...
  int32_t i,imax,j,k;
  FLOAT_LIBVISO   big,dum,sum,temp;
  FLOAT_LIBVISO* vv = (FLOAT_LIBVISO*)malloc(n*sizeof(FLOAT_LIBVISO)); // vv stores the implicit scaling of each row.
  d = 1.0;
...
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  FLOAT_LIBVISO* w   = (FLOAT_LIBVISO*)malloc(n*sizeof(FLOAT_LIBVISO));
  FLOAT_LIBVISO* rv1 = (FLOAT_LIBVISO*)malloc(n*sizeof(FLOAT_LIBVISO));

  int32_t flag,i,its,j,jj,k,l,nm;
  FLOAT_LIBVISO   anorm,c,f,g,h,s,scale,x,y,z;
  ...
  for (k=n-1;k>=0;k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
    for (its=0;its<30;its++) { // and over allowed iterations.
      ...
      for (l=k;l>=0;l--) { // Test for splitting.
        nm = l-1;
        if ((FLOAT_LIBVISO)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
        if ((FLOAT_LIBVISO)(fabs( w[nm])+anorm) == anorm) { break; }
      }
      if (flag) {
        ...
        for (i=l;i<=k;i++) {
          f = s*rv1[i];
          rv1[i] = c*rv1[i];
          if ((FLOAT_LIBVISO)(fabs(f)+anorm) == anorm) break;
          g = w[i];
          h = pythag(f,g);
          w[i] = h;
          h = 1.0/h;
  ...
  int32_t s2,inc=1;
  FLOAT_LIBVISO   sw;
  FLOAT_LIBVISO* su = (FLOAT_LIBVISO*)malloc(m*sizeof(FLOAT_LIBVISO));
  FLOAT_LIBVISO* sv = (FLOAT_LIBVISO*)malloc(n*sizeof(FLOAT_LIBVISO));
...
void Matrix::allocateMemory (const int32_t m_,const int32_t n_) {
  ...
  val    = (FLOAT_LIBVISO**)malloc(m*sizeof(FLOAT_LIBVISO*));
  val[0] = (FLOAT_LIBVISO*)calloc(m*n,sizeof(FLOAT_LIBVISO));
...
FLOAT_LIBVISO Matrix::pythag(FLOAT_LIBVISO a,FLOAT_LIBVISO b) {
  FLOAT_LIBVISO absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
