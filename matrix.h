...
typedef double FLOAT_LIBVISO;      // double precision
...
public:
  ...
  Matrix (const int32_t m,const int32_t n,const FLOAT_LIBVISO* val_); // init mxn matrix with values from array 'val'
  ...
  void getData(FLOAT_LIBVISO* val_,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
  ...
  void setVal(FLOAT_LIBVISO s,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(FLOAT_LIBVISO s,int32_t i1=0,int32_t i2=-1);
  ...
  static Matrix rotMatX(const FLOAT_LIBVISO &angle);
  static Matrix rotMatY(const FLOAT_LIBVISO &angle);
  static Matrix rotMatZ(const FLOAT_LIBVISO &angle);
  ...
  Matrix  operator* (const FLOAT_LIBVISO &s);  // multiply with scalar
  Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
  Matrix  operator/ (const FLOAT_LIBVISO &s);  // divide by scalar
  Matrix  operator- ();                // negative matrix
  Matrix  operator~ ();                // transpose
  FLOAT_LIBVISO   l2norm ();                   // euclidean norm (vectors) / frobenius norm (matrices)
  FLOAT_LIBVISO   mean ();                     // mean of all elements in matrix
  ...
  FLOAT_LIBVISO  det ();                                             // returns determinant of matrix
  bool   solve (const Matrix &M,FLOAT_LIBVISO eps=1e-20);            // solve linear system M*x=B, replaces *this and M
  bool   lu(int32_t *idx, FLOAT_LIBVISO &d, FLOAT_LIBVISO eps=1e-20);        // replace *this by lower upper decomposition
...
private:
  ...
  void releaseMemory ();
  inline FLOAT_LIBVISO pythag(FLOAT_LIBVISO a,FLOAT_LIBVISO b);
